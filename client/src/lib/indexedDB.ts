...
export const getChatHistory = async (): Promise<IDBMessage[]> => {
  try {
    const conversation = await indexedDBService.getMostRecentConversation();
...
export const saveMessageToDB = async (message: any): Promise<void> => {
  try {
    // Get or create a conversation
    let conversation = await indexedDBService.getMostRecentConversation();
    
    if (!conversation) {
      // Create a new conversation
      const conversationId = await indexedDBService.createConversation('Wine Chat');
      conversation = await indexedDBService.getConversation(conversationId);
    }
    
    if (conversation && conversation.id) {
      // Add message to the conversation
      await indexedDBService.addMessageToConversation(conversation.id, {
        role: message.role,
