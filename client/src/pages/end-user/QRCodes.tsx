...
const QRCodes: React.FC = () => {
  ...
  return (
    ...
    >
      <AppHeader title='QR Codes for Deployment' showBackButton={true} onBack={() => window.history.back()} showMyCellarLink={true} />

      <div className='container mx-auto px-4 py-8 pt-24'>
        <div className='max-w-4xl mx-auto'>
          <div className='text-center mb-8'>
            <h1 className='text-3xl font-bold text-white mb-4'>Deployment QR Codes</h1>
            <p className='text-gray-300 text-lg'>QR codes for the main pages of your wine application</p>
            <p className='text-gray-400 text-sm mt-2'>After deployment, update the domain in the code to generate working QR codes</p>
          </div>

          <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6'>
            {pages.map((page, index) => (
              <div key={index} className='bg-black/30 backdrop-blur-sm border border-white/20 rounded-lg p-6 text-center'>
                ...
                <div className='bg-white p-4 rounded-lg mb-4 inline-block'>
                  <QRCodeSVG value={page.url} size={150} level='M' includeMargin={true} />
                </div>
          ...
          <div className='mt-12 bg-black/40 backdrop-blur-sm border border-white/20 rounded-lg p-6'>
            <h2 className='text-xl font-semibold text-white mb-4'>Deployment Instructions</h2>
          ...
          <div className='mt-8 bg-black/40 backdrop-blur-sm border border-white/20 rounded-lg p-6'>
            <h2 className='text-xl font-semibold text-white mb-4'>QR Code Usage</h2>
            <div className='text-gray-300 space-y-2'>
              <p>
                <strong>Main Wine Page:</strong> Primary QR for wine bottles - triggers QR scan modal
              </p>
              <p>
                <strong>Wine Cellar:</strong> Direct access to user's wine collection
              </p>
              <p>
                <strong>Scanned Wine:</strong> Alternative entry point for scanned wines
              </p>
              <p>
                <strong>Specific Wine:</strong> Direct link to a particular wine's details
              </p>
              <p>
                <strong>Global Home:</strong> General landing page
              </p>
              <p>
                <strong>Wine Conversation:</strong> Direct access to chat interface
              </p>
            </div>
          </div>
        </div>
