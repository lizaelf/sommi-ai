...
const FoodPairingSuggestionsPage: React.FC = () => {
  ...
  return (
    ...
    >
      <AppHeader showBackButton={true} onBack={() => window.history.back()} title={currentWine?.name || ''} />
      <HeaderSpacer />
      {/* Content */}
      <div style={{ padding: '24px 16px', flex: 1 }}>
        {isLoading && (
          ...
          >
            ...
            <p style={{ ...typography.body, color: 'rgba(255, 255, 255, 0.8)' }}>Generating AI food pairings...</p>
          </div>
        )}
        ...
        {error && (
          ...
          >
            <p style={{ ...typography.body, color: '#EF4444', textAlign: 'center' }}>{error}</p>
            <Button variant='primary' onClick={generateFoodPairings} style={{ padding: '12px 24px' }}>
              Try Again
            </Button>
          </div>
        )}
              ...
              >
                ...
                >
                  <div style={{ flex: 1 }}>
                    ...
                    >
                      <span role='img' aria-label='dish-emoji' style={{ fontSize: '1.1em', marginRight: '6px' }}>
                        {getDishEmoji(pairing.dish)}
                      </span>
                      {pairing.dish}
                    </h3>
                  </div>
        ...
        {!isLoading && !error && foodPairings.length === 0 && currentWine && (
          ...
          >
            <p style={{ ...typography.body, color: 'rgba(255, 255, 255, 0.6)', textAlign: 'center' }}>No food pairings generated yet.</p>
            <Button variant='primary' onClick={generateFoodPairings} style={{ padding: '12px 24px' }}>
              Generate Pairings
            </Button>
          </div>
