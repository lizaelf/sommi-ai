...
const TenantAdmin: React.FC = () => {
          ...
          <div className='p-6'>
            ...
            <div className='flex items-center gap-4 mb-6'>
              <div className='flex-1 relative'>
                ...
                <input type='text' placeholder='Search wines...' value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className='w-full pl-12 pr-4 py-4 bg-white/5 border border-white/20 rounded-lg text-white placeholder-white/40 focus:border-white/40 focus:outline-none' />
              </div>
              <Button variant='primary' onClick={() => setLocation('/wine-edit/new')} className='flex items-center gap-2 whitespace-nowrap'>
            ...
            <div className='space-y-4'>
              {filteredWines.map(wine => (
                <div key={wine.id} className='flex items-center gap-4 p-4 bg-white/5 border border-white/20 rounded-lg hover:bg-white/8 transition-colors cursor-pointer' onClick={() => handleEditWine(wine)}>
                  {/* Wine Image */}
                  <div className='w-16 h-20 flex-shrink-0'>
                    <WineImage wineName={wine.name} alt={wine.name} style={{ width: '100%', height: '100%', objectFit: 'contain' }} />
                  </div>
  ...
  return (
    <div className='min-h-screen bg-black text-white mx-auto' style={{ maxWidth: '1200px' }}>
      <AppHeader title={currentTenant?.name || 'Admin Panel'} showBackButton={true} rightContent={<ActionDropdown actions={actions} />} />
      <HeaderSpacer />
