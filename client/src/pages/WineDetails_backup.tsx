...
export default function WineDetails({ selectedWine: propSelectedWine }: WineDetailsProps) {
  const [location] = useLocation();
  const { id } = useParams();
  const [wine, setWine] = useState<SelectedWine | null>(propSelectedWine || null);
  ...
  useEffect(() => {
    if (id && !wine) {
      const wineData = storage.getWineById(parseInt(id));
      if (wineData) {
        setWine(wineData);
      }
    }
