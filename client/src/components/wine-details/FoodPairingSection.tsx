...
const FoodPairingSection: React.FC<FoodPairingSectionProps> = ({ foodPairing, wineId, wineName }) => {
  ...
  const { id } = useParams()
  const handleSeeAllClick = (id: string) => {
    setLocation(`/${tenantName}/food-pairings-ai/${id}`)
  }
  ...
  return (
    <div style={{ padding: '0 16px', marginBottom: '32px' }}>
      ...
      >
        ...
        <SectionHeaderButton onClick={() => handleSeeAllClick(id || '')}>See all</SectionHeaderButton>
      </div>
      ...
      >
        {foodPairingCards.map((card: any, index: number) => (
          <FoodPairingCard key={index} image={card.image} title={card.title} description={card.description} />
        ))}
      </div>
    </div>
  )
