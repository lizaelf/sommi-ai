...
const VoiceAssistant: React.FC<VoiceAssistantProps> = ({
  ...
  useEffect(() => {
    ...
    const handleTriggerVoiceAssistant = async () => {
      ...
      if (isManuallyClosedRef.current) {
        console.log(
          "QR SCAN: Voice assistant manually closed, ignoring trigger",
        );
        return;
      }
      ...
      console.log(
        "QR SCAN: Voice button clicked - proceeding with voice assistant",
      );
      ...
      if (isDeployment) {
        ...
        const voiceLocked =
          (window as any).VOICE_LOCK_VERIFIED &&
          (window as any).GUARANTEED_MALE_VOICE;
        ...
        if (!voiceLocked) {
          console.log(
            "🚫 DEPLOYMENT: Blocking welcome message - voice not verified",
          );
          // Wait a moment for voice system to initialize, then try again
          setTimeout(() => {
            const retryVoiceLocked =
              (window as any).VOICE_LOCK_VERIFIED &&
              (window as any).GUARANTEED_MALE_VOICE;
            if (retryVoiceLocked) {
              console.log(
                "✅ DEPLOYMENT: Voice verified on retry, showing welcome",
              );
              ...
              console.log(
                "🚫 DEPLOYMENT: Voice still not verified after retry",
              );
              setShowBottomSheet(true);
              sessionStorage.setItem("voice_bottom_sheet_shown", "true");
              setShowAskButton(true);
      ...
      if (welcomeAudioElementRef.current) {
        ...
        audio.onended = () => {
          if (!isManuallyClosedRef.current) {
            ...
            setIsThinking(false);
            console.log(
              "QR SCAN: Welcome message completed - showing suggestions",
            );
          }
        ...
        try {
          ...
          console.error(
            "QR SCAN: Audio playback failed, generating fresh audio:",
            error,
          );
          // fallback logic...
        }
          ...
          .then((buffer) => {
            ...
            audio.onended = () => {
              ...
              if (!isManuallyClosedRef.current) {
                ...
                setIsThinking(false);
                console.log(
                  "QR SCAN: Fallback welcome message completed - showing suggestions",
                );
              }
              URL.revokeObjectURL(audioUrl);
              (window as any).currentOpenAIAudio = null;
            ...
            audio.onerror = () => {
              ...
              if (!isManuallyClosedRef.current) {
                ...
                setIsThinking(false);
                console.log(
                  "QR SCAN: Fallback audio error - showing suggestions",
                );
              }
              URL.revokeObjectURL(audioUrl);
              (window as any).currentOpenAIAudio = null;
    ...
    const handlePlayAudioResponse = async (event: Event) => {
      ...
      if (isTextOnly || context === "chat") {
        console.log(
          "🚫 VOICE ASSISTANT: Blocking text-only/chat suggestion from triggering audio",
        );
        return;
      }
    ...
    window.addEventListener(
      "suggestionPlaybackStarted",
      handleSuggestionPlayback,
    );
    window.addEventListener(
      "suggestionPlaybackEnded",
  ...
  const checkVoiceActivity = () => {
    ...
    if (
      ...
      max === 0 &&
      timeAverage === 128 &&
      consecutiveSilenceCountRef.current % 100 === 0
    ) {
  ...
  const startListening = async () => {
    console.log(
      "🎤 DEPLOY DEBUG: Starting audio recording for Whisper transcription",
    );

    // Check if manually closed - don't reopen if so
    if (isManuallyClosedRef.current) {
      console.log(
        "🎤 DEPLOY DEBUG: Voice assistant manually closed, ignoring startListening",
      );
      return;
    }
    ...
    console.log(
      "🎤 DEPLOY DEBUG: Setting UI states - showBottomSheet: true, isListening: true",
    );
    setShowBottomSheet(true);
    setIsListening(true);
    ...
    console.log(
      '🎤 VoiceAssistant: Dispatching mic-status "listening" event (early)',
    );
    const micEvent = new CustomEvent("mic-status", {
      detail: { status: "listening" },
    });
    console.log(
      "🎤 VoiceAssistant: Created event:",
      micEvent,
      "detail:",
      micEvent.detail,
    );
  ...
  const setupRecording = async () => {
    ...
    try {
        ...
        (async () => {
          try {
            ...
            if (audioBlob.size < 1000) {
              // Less than 1KB indicates no meaningful audio
            ...
            console.log(
              '🎤 VoiceAssistant: Dispatching mic-status "stopped" event',
            );
            window.dispatchEvent(
      ...
      setTimeout(() => {
        console.log(
          '🎤 VoiceAssistant: Dispatching mic-status "listening" event (confirmation)',
        );
        window.dispatchEvent(
          new CustomEvent("mic-status", {
            detail: { status: "listening" },
      ...
      // Emit microphone status event for wine bottle animation
      console.log(
        '🎤 VoiceAssistant: Dispatching mic-status "listening" event',
      );
      window.dispatchEvent(
        new CustomEvent("mic-status", {
      ...
      setTimeout(() => {
        if (
          ...
          console.log(
            "Secondary backup auto-stop after 10 seconds - silence detection failed",
          );
          stopListening();
        }
      }, 10000);
      ...
      if (error instanceof Error && error.name === "NotAllowedError") {
        ...
        toastError("Failed to start voice recording");
      }
    }
  };
  ...
  const handleSuggestionClick = (
    ...
    console.log(
      "VoiceAssistant: Suggestion clicked:",
      suggestion,
      "with options:",
      options,
    );
    ...
    if (options?.textOnly) {
      console.error(
        "🚨 CONTEXT LEAK DETECTED: Chat suggestion reached voice assistant!",
      );
      console.error(
        "This indicates improper routing - suggestion should have been handled by chat interface",
      );
    ...
    console.log(
      "🚀 VoiceAssistant: SuggestionPills handles all suggestion logic - voice assistant does nothing",
    );
  ...
  const handleCloseBottomSheet = () => {
    console.log(
      "VoiceAssistant: Manual close triggered - setting flag to prevent reopening",
    );
    ...
    console.log(
      "VoiceAssistant: Permanently closed - no automatic reopening until page refresh",
    );
  };
  ...
  const handleUnmute = async () => {
    ...
    console.log(
      "Current window.lastAssistantMessageText:",
      (window as any).lastAssistantMessageText,
    );
    ...
    try {
      ...
      if (pendingAudio) {
        ...
        pendingAudio.onended = () => {
          ...
          setShowAskButton(true);
          console.log(
            "Pending audio playback completed successfully - Ask button enabled",
          );
        };
      ...
      if (!lastAssistantMessage || !lastAssistantMessage.trim()) {
        console.warn(
          "No assistant message available to play - attempting comprehensive fallback",
        );
        ...
        if (assistantElements.length > 0) {
          ...
          fallbackText =
            lastElement.textContent || (lastElement as HTMLElement).innerText;
        }

        // Strategy 2: Look for assistant message containers with specific classes
        if (!fallbackText || !fallbackText.trim()) {
          const messageContainers = document.querySelectorAll(
            '.message-assistant, .assistant-message, [class*="assistant"]',
          );
          for (let i = messageContainers.length - 1; i >= 0; i--) {
            const container = messageContainers[i];
            const text =
              container.textContent || (container as HTMLElement).innerText;
        ...
        if (!fallbackText || !fallbackText.trim()) {
          const allMessages = document.querySelectorAll(
            '[class*="message"], [data-message], .prose',
          );
        ...
        if (fallbackText && fallbackText.trim()) {
          console.log(
            "Using fallback message from comprehensive search:",
          ...
          console.warn(
            "No assistant message available to play - all fallback strategies failed",
          );
          // Instead of returning, use a default welcome message for unmute
          const defaultMessage =
            "I'm here to help you explore this wine. What would you like to know?";
      ...
      console.log(
        ...
        finalMessageText
          ? finalMessageText.substring(0, 50) + "..."
      ...
      try {
        ...
        try {
          ...
          const selectVoice = () => {
            ...
            if (lockedVoiceURI) {
              const lockedVoice = voices.find(
                (voice) => voice.voiceURI === lockedVoiceURI,
              );
              if (lockedVoice) {
        ...
        audio.onended = () => {
          ...
          (window as any).currentOpenAIAudio = null;
          console.log(
            "Manual unmute TTS playback completed successfully - Ask button enabled",
          );
        };
        ...
        audio.onerror = (e) => {
          ...
          console.log(
            "DEPLOYMENT FIX: OpenAI audio failed, using browser TTS as fallback",
          );
          ...
          utterance.onend = () => {
            ...
            setShowAskButton(true);
            console.log(
              "DEPLOYMENT FIX: Browser TTS fallback completed - Ask button enabled",
            );
          };
        ...
        try {
          ...
          if (playPromise !== undefined) {
            await playPromise;
            console.log(
              "Manual unmute audio play promise resolved successfully",
            );
          }
        } catch (playError: any) {
          ...
          console.log(
            "DEPLOYMENT FIX: OpenAI audio play failed, using browser TTS fallback",
          );
          ...
          utterance.onstart = () => {
            ...
            setShowAskButton(false);
            console.log(
              "DEPLOYMENT FIX: Browser TTS fallback started for play error",
            );
          };
