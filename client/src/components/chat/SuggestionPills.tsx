...
export default function SuggestionPills({ wineKey, conversationId, onSuggestionClick, isDisabled = false, preferredResponseType = 'text', context = 'chat' }: SuggestionPillsProps) {
  ...
  const {
    ...
    queryFn: async () => {
      const response = await fetch(`/api/suggestion-pills/${encodeURIComponent(effectiveWineKeyForQuery)}`)
      if (!response.ok) {
  ...
  const preGenerateSuggestionAudio = async (suggestions?: SuggestionPill[]) => {
    ...
    for (const pill of pillsToProcess.slice(0, 3)) {
      ...
      const cacheKey = getSuggestionCacheKey(String(effectiveWineKey), suggestionId, 'audio')
  ...
  const handlePillClick = async (pill: SuggestionPill) => {
    ...
    try {
      ...
      if (!instantResponse) {
        instantResponse = await suggestionCache.getCachedResponse(String(effectiveWineKey), suggestionId)
      }
      console.log('💾 Cached response found:', !!instantResponse, 'Context:', context, 'PillId:', pill.id, 'PreferredType:', preferredResponseType)

      // CHAT CONTEXT: Handle text-only, no audio
      if (context === 'chat') {
        console.log('💬 CHAT CONTEXT: Processing suggestion for chat interface')

        onSuggestionClick?.(pill.text)
  ...
  const visiblePills = useMemo(() => {
    ...
    const sourcePills = suggestionsData?.suggestions && !isLoading ? suggestionsData.suggestions : defaultSuggestions
    ...
    const availablePills = sourcePills.filter((pill: SuggestionPill) => !usedPills.has(pill.id))
