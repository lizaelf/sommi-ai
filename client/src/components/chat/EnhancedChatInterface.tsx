...
import { useConversation, useWineConversation } from '@/hooks/UseConversation'
...
const EnhancedChatInterface: React.FC<EnhancedChatInterfaceProps> = ({ showBuyButton = false, selectedWine = null, onReady, isScannedPage = false, tenantName }) => {
  ...
  const { conversationKey, messages, addMessage, createNewConversation, clearConversation, refetchMessages } = useWineConversation(tenantName, selectedWine)

  // Clear old conversation data if wine doesn't match stored messages
  useEffect(() => {
    if (selectedWine && messages.length > 0) {
      const hasOldRidgeContent = messages.some(msg => msg.content.includes('Ridge "Lytton Springs"') && !selectedWine.name.includes('Ridge'))

      if (hasOldRidgeContent) {
        console.log('Clearing outdated conversation data for wine:', selectedWine.name)
        clearConversation()
      }
    }
  }, [selectedWine, messages, clearConversation])
    ...
    <div className='flex flex-col h-auto' style={{ width: '100%' }}>
      ...
      <div className='flex flex-1 overflow-hidden'>
        ...
        >
                  ...
                  <>
                    ...
                    {messages.map((message: any, index: number) => (
                      ...
                      >
                        ...
                        >
                          <ChatAnswer content={message.content} isUserMessage={message.role === 'user'} />
                        </div>
                      </div>
                    ))}
                  </>
          ...
          <ChatInputArea currentWine={currentWine} currentConversationId={conversationKey as any} isTyping={isTyping} onSendMessage={handleSendMessage} onSuggestionClick={handleSuggestionClick} onKeyboardFocus={(focused: boolean) => setIsKeyboardFocused(focused)} onMicClick={handleMicClick} />
        </main>
      </div>

      {/* VoiceController — один раз на сторінку, слухає події, керує всім флоу */}
      <VoiceController ref={voiceControllerRef} onSendMessage={handleSendMessage} onAddMessage={handleAddMessage} isProcessing={isTyping} conversationId={conversationKey} />
    </div>
  )
}
