...
import { adaptIDBMessagesToMessages, adaptIDBConversationsToConversations, adaptMessageToIDBMessage } from '@/lib/adapters'
import { generateConversationKey } from '@/utils/deviceId'
...
export function useConversation(wineId?: string | number): UseConversationReturn {
  ...
  const createNewConversation = useCallback(async (): Promise<number | null> => {
    try {
      ...
      const wineTitle = wineId && wineId !== 'default' ? `Wine ${String(wineId).replace('wine_', '')} Conversation` : `New Conversation`
  ...
  const addMessage = useCallback(
    async (message: Message | ClientMessage) => {
      ...
      try {
        ...
        try {
          await indexedDBService.addMessageToConversation(currentConversationId, adaptMessageToIDBMessage(message))
...
interface UseWineConversationReturn {
...
export function useWineConversation(tenantName?: string, wine?: Wine | null): UseWineConversationReturn {
  const [conversationKey, setConversationKey] = useState<string | null>(null)
  const [messages, setMessages] = useState<any[]>([])

  // Генеруємо ключ розмови
  const generateKey = useCallback(() => {
    if (!tenantName || wine?.id === undefined || wine?.id === null) return null
    return generateConversationKey(tenantName, wine.id)
  }, [tenantName, wine?.id])
  ...
  const loadConversation = useCallback(async (key: string) => {
    try {
      ...
      const conversationResponse = await fetch(`/api/wine-conversations/${key}`)

      if (conversationResponse.ok) {
        // Завантажуємо повідомлення
        const messagesResponse = await fetch(`/api/wine-conversations/${key}/messages`)
  ...
  const createNewConversation = useCallback(async (): Promise<string | null> => {
    ...
    try {
      const response = await fetch('/api/wine-conversations', {
        method: 'POST',
        ...
        body: JSON.stringify({
          ...
          title: `Chat about ${wine?.name || 'wine'}`,
        }),
      })
  ...
  const addMessage = useCallback(
    async (message: any) => {
      ...
      try {
        const response = await fetch('/api/wine-messages', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            conversationKey,
  ...
  // Перезавантаження повідомлень
  const refetchMessages = useCallback(async () => {
    if (!conversationKey) return null

    try {
      const response = await fetch(`/api/wine-conversations/${conversationKey}/messages`)
      if (response.ok) {
