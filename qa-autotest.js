...
class QATestSuite {
  ...
  async testAPIStatus() {
    ...
    this.logTest(
      'API Status',
      response.status === 200,
      response.status === 200 ? 'API endpoints available' : `Status: ${response.status}`
    );
  }
  ...
  async testWineDataIntegrity() {
    try {
      ...
      // Test wine configuration import
      if (wineConfigExists) {
        const wineConfig = require('./shared/wineConfig.ts');
        this.logTest(
          'Wine Config Structure',
          wineConfig.WINE_CONFIG && wineConfig.WINE_CONFIG.name,
          'Wine configuration valid'
        );
      }
    } catch (error) {
  ...
  async testSuggestionPillsAPI() {
    ...
    this.logTest(
      ...
      response.status === 200 ? 'Suggestion pills loading' : `Status: ${response.status}`
    );
  }
  ...
  async testEnvironmentConfig() {
    try {
      ...
      // Test if server can access environment variables
      const response = await this.makeRequest('/api/status');
      this.logTest(
        'Environment Variables',
        response.status === 200,
        'Server can access environment'
      );
    } catch (error) {
  ...
  async testPerformanceMetrics() {
    try {
      ...
      this.logTest(
        'Response Time',
        responseTime < 2000,
        `${responseTime}ms ${responseTime < 1000 ? '(Excellent)' : responseTime < 2000 ? '(Good)' : '(Slow)'}`
      );
